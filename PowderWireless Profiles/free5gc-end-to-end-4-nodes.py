#!/usr/bin/env python
"""
This profile is the base for an end-to-end 5G network with 4 nodes.

One node for the 5G core, one for the UPF function, one for the
simulated Radio Access Network (RAN), and one for the UE.
"""
...
# Add node that will host the 5G Core UPF Virtual Network Function.
upf = request.RawPC("upf")
...
# Add node that will host the UE.
ue = request.RawPC("ue")
ue.component_manager_id = GLOBALS.SITE_URN
ue.disk_image = GLOBALS.UBUNTU18_IMG
ue.hardware_type = GLOBALS.HWTYPE
gNBCoreLink.addNode(ue)
